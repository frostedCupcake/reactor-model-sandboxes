# SANA-Streaming sandbox

A complete, independently runnable recreation of the launchd SANA-Streaming sandbox, with separate frontend and backend services.

- Live reference: [thelaunchd.com/reactor/models/sana-streaming](https://thelaunchd.com/reactor/models/sana-streaming)
- Reactor documentation: [SANA-Streaming model reference](https://docs.reactor.inc/model-api-reference/sana-streaming/overview)
...
cd reactor-model-sandboxes/sana-streaming
npm install
```
...
2. Choose a webcam or uploaded video clip and enter an edit prompt or use a preset.
...
7. Apply another prompt while streaming. Clear an uploaded clip to stop its tracks and return to webcam mode.
...
- **Connecting never becomes Live:** Verify the key can access SANA-Streaming, required media is selected, and the backend minted a token for the exact model scope.
