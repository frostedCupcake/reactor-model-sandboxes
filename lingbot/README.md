...
A complete, independently runnable recreation of the launchd LingBot sandbox, with separate frontend and backend services.

- Live reference: [thelaunchd.com/reactor/models/lingbot](https://thelaunchd.com/reactor/models/lingbot)
- Reactor documentation: [LingBot model reference](https://docs.reactor.inc/model-api-reference/lingbot/overview)
...
cd reactor-model-sandboxes/lingbot
npm install
```
...
2. Select or upload a required reference image and enter the world prompt.
...
7. Use WASD and the arrow keys, change rotation speed, update the reference, or apply a new prompt.
...
- **Connecting never becomes Live:** Verify the key can access LingBot, required media is selected, and the backend minted a token for the exact model scope.
