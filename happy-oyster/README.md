# Happy Oyster sandbox

A complete, independently runnable recreation of the launchd Happy Oyster sandbox, with separate frontend and backend services.

- Live reference: [thelaunchd.com/reactor/models/happy-oyster](https://thelaunchd.com/reactor/models/happy-oyster)
- Reactor documentation: [Happy Oyster model reference](https://docs.reactor.inc/model-api-reference/happy-oyster/overview)
...
cd reactor-model-sandboxes/happy-oyster
npm install
```
...
2. Choose Adventure or Directing mode, optionally select a first-frame reference, and describe the world.
...
7. In Adventure mode, use WASD and the arrow keys. In Directing mode, apply new text instructions while the world runs.
...
- **Connecting never becomes Live:** Verify the key can access Happy Oyster, required media is selected, and the backend minted a token for the exact model scope.
