...
A complete, independently runnable recreation of the launchd X2 sandbox, with separate frontend and backend services.

- Live reference: [thelaunchd.com/reactor/models/x2](https://thelaunchd.com/reactor/models/x2)
- Reactor documentation: [X2 model reference](https://docs.reactor.inc/model-api-reference/x2/overview)
...
2. Choose a webcam or uploaded video clip, select or upload a reference image, and enter an edit prompt.
...
7. Drag or click on the output to send pointer coordinates. Replace the reference while the session is live.
...
- **Connecting never becomes Live:** Verify the key can access X2, required media is selected, and the backend minted a token for the exact model scope.
