...
A complete, independently runnable recreation of the launchd LongLive 2 sandbox, with separate frontend and backend services.

- Live reference: [thelaunchd.com/reactor/models/longlive-v2](https://thelaunchd.com/reactor/models/longlive-v2)
- Reactor documentation: [LongLive 2 model reference](https://docs.reactor.inc/model-api-reference/longlive-v2/overview)
...
cd reactor-model-sandboxes/longlive-v2
npm install
```
...
2. Enter the opening-shot prompt before starting the session.
...
7. Enter another prompt to create a hard scene cut. Reuse earlier prompts from the local timeline.
...
- **Connecting never becomes Live:** Verify the key can access LongLive 2, required media is selected, and the backend minted a token for the exact model scope.
